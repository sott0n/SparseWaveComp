// RUN: sparsewave-opt %s \
// RUN:   --pass-pipeline='builtin.module(sparsewave-to-amdgpu-pipeline{chip=gfx942 wavefront-size=64 binary-format=none spmv-mapping=thread-per-row spmv-block-size=128})' \
// RUN:   | FileCheck %s
// RUN: sparsewave-opt %s \
// RUN:   --pass-pipeline='builtin.module(sparsewave-to-amdgpu-pipeline{chip=gfx942 wavefront-size=32 binary-format=none spmv-mapping=wave-per-row spmv-block-size=128})' \
// RUN:   | FileCheck %s --check-prefix=WAVE
// RUN: sparsewave-opt %s \
// RUN:   --pass-pipeline='builtin.module(sparsewave-to-amdgpu-pipeline{chip=gfx942 wavefront-size=32 binary-format=none spmv-mapping=block-per-row spmv-block-size=128})' \
// RUN:   | FileCheck %s --check-prefix=BLOCK
// RUN: sparsewave-opt %s \
// RUN:   --pass-pipeline='builtin.module(sparsewave-to-amdgpu-pipeline{chip=gfx942 wavefront-size=64 binary-format=none spmv-mapping=thread-per-position spmv-block-size=128})' \
// RUN:   | FileCheck %s --check-prefix=POSITION
// RUN: sparsewave-opt %s \
// RUN:   --pass-pipeline='builtin.module(sparsewave-to-amdgpu-pipeline{chip=gfx942 wavefront-size=32 binary-format=none spmv-mapping=wave-per-position spmv-block-size=128})' \
// RUN:   | FileCheck %s --check-prefix=SEGMENT

// CHECK-NOT: sparsewave.spmv
// CHECK-LABEL: func.func @spmv(
// CHECK: %[[BLOCK_SIZE:.*]] = arith.constant 128 : index
// CHECK: gpu.launch_func @spmv_kernel::@spmv_kernel
// CHECK-SAME: threads in (%[[BLOCK_SIZE]],
// CHECK-LABEL: gpu.module @spmv_kernel
// CHECK-SAME: [#rocdl.target<chip = "gfx942">]
// CHECK: llvm.func @spmv_kernel(
// CHECK-SAME: rocdl.kernel
// CHECK: rocdl.workgroup.id.x
// CHECK: rocdl.workitem.id.x
// CHECK-NOT: scf.
// WAVE-NOT: sparsewave.spmv
// WAVE-LABEL: gpu.module @spmv_kernel
// WAVE-SAME: [#rocdl.target<chip = "gfx942"
// WAVE-SAME: flags = {no_wave64}
// WAVE: llvm.func @spmv_kernel(
// WAVE-SAME: rocdl.kernel
// WAVE: rocdl.ds_bpermute
// WAVE-NOT: scf.
// BLOCK-NOT: sparsewave.spmv
// BLOCK-LABEL: gpu.module @spmv_kernel
// BLOCK-SAME: [#rocdl.target<chip = "gfx942"
// BLOCK-SAME: flags = {no_wave64}
// BLOCK: llvm.mlir.global internal @__wg_spmv_kernel_0()
// BLOCK-SAME: {addr_space = 3 : i32} : !llvm.array<4 x f32>
// BLOCK: llvm.func @spmv_kernel(
// BLOCK-SAME: rocdl.kernel
// BLOCK: rocdl.ds_bpermute
// BLOCK: rocdl.s.barrier
// BLOCK-NOT: scf.
// POSITION-NOT: sparsewave.
// POSITION-LABEL: gpu.module @spmv_kernel
// POSITION: llvm.func @spmv_kernel(
// POSITION-SAME: rocdl.kernel
// POSITION: llvm.atomicrmw fadd
// POSITION-NOT: scf.
// SEGMENT-NOT: sparsewave.
// SEGMENT-LABEL: gpu.module @spmv_kernel
// SEGMENT-SAME: [#rocdl.target<chip = "gfx942"
// SEGMENT-SAME: flags = {no_wave64}
// SEGMENT: llvm.func @spmv_kernel(
// SEGMENT-SAME: rocdl.kernel
// SEGMENT: rocdl.ds_bpermute
// SEGMENT: llvm.atomicrmw fadd
// SEGMENT-NOT: scf.
func.func @spmv(
    %rowOffsets: memref<?xi32>,
    %columnIndices: memref<?xi32>,
    %values: memref<?xf32>,
    %vector: memref<?xf32>,
    %output: memref<?xf32>) {
  sparsewave.spmv %rowOffsets, %columnIndices, %values, %vector, %output
      : memref<?xi32>, memref<?xi32>, memref<?xf32>, memref<?xf32>,
        memref<?xf32>
  return
}
